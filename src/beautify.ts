export default function beautify(options: any): string {
  const data: data = options.parsed,
    lexer: string = "markup",
    c: number =
      options.end < 1 || options.end > data.token.length ? data.token.length : options.end + 1,
    lf: "\r\n" | "\n" = options.crlf === true ? "\r\n" : "\n",
    externalIndex: externalIndex = {},
    levels: number[] = (function beautify_markup_levels(): number[] {
      const level: number[] =
          options.start > 0 ? Array(options.start).fill(0, 0, options.start) : [],
        nextIndex = function beautify_markup_levels_next(): number {
          ...
          if (
            ...
            (a < c - 1 && data.types[x].indexOf("attribute") > -1)
          ) {
            do {
        ...
        content = function beautify_markup_levels_content(): void {
          ...
          if (
            next < c &&
            (data.types[next].indexOf("end") > -1 || data.types[next].indexOf("start") > -1) &&
            data.lines[next] > 0
          ) {
            level.push(indent);
            ind = ind + 1;
              ...
              do {
                ...
                if (
                  (data.types[d] !== "attribute" ||
                    (data.types[d] === "attribute" && data.types[d + 1] !== "attribute")) &&
                  data.lines[d + 1] > 0
                ) {
                  if (
                    data.types[d] !== "singleton" ||
                    (data.types[d] === "singleton" && data.types[d + 1] !== "attribute")
                  ) {
                    count = data.token[a].length;
                    if (data.lines[a + 1] > 0) {
                      count = count + 1;
      ...
      let a: number = options.start,
        ...
        next: number = 0,
        count: number = 0,
        indent: number = isNaN(options.indent_level) === true ? 0 : Number(options.indent_level);
          ...
          if (data.types[a].indexOf("attribute") > -1) {
            ...
            if (data.types[a] === "script_end" && data.types[a + 1] === "end") {
              ...
              (options.force_indent === false ||
                (options.force_indent === true && data.types[next] === "script_start")) &&
              ...
              if (data.lines[next] > 0 && data.types[next] === "script_start") {
                ...
                next < c &&
                (data.types[next].indexOf("end") > -1 || data.types[next].indexOf("start") > -1) &&
                (data.lines[next] > 0 || data.types[next].indexOf("template_") > -1)
              ) {
                level.push(indent);
              } else if (data.lines[next] === 0) {
                level.push(-20);
              ...
              if (options.language === "jsx" && data.token[a + 1] === "{") {
                ...
                (data.types[next] === "content" ||
                  ...
                  (data.types[a] === "start" && data.types[next] === "template"))
              ) {
                level.push(-20);
              } else {
                level.push(indent);
              ...
              options.force_indent === false &&
              data.lines[next] === 0 &&
              (data.types[next] === "content" || data.types[next] === "singleton")
            ) {
              level.push(-20);
  ...
  return (function beautify_markup_apply(): string {
    const build: string[] = [],
      ...
      attributeEnd = function beautify_markup_apply_attributeEnd(): void {
        ...
        let y: number = a + 1,
          space: string =
            options.space_close === true && end !== null && end[0] === "/>" ? " " : "",
          jsx: boolean = false;
        if (end === null) {
          return;
        }
        ...
        do {
          if (data.types[y] === "jsx_attribute_end" && data.begin[data.begin[y]] === a) {
            ...
            jsx === false &&
            (data.begin[y] < a || data.types[y].indexOf("attribute") < 0)
          ) {
            break;
          }
          y = y + 1;
    ...
    do {
      if (data.lexer[a] === lexer || options.beautify[data.lexer[a]] === undefined) {
        if (
          (data.types[a] === "start" ||
            data.types[a] === "singleton" ||
            data.types[a] === "xml" ||
        ...
        if (
          ...
          ((data.types[a] === "content" && options.preserve_text === false) ||
        ...
        if (externalIndex[a] === a && data.types[a] !== "reference") {
          ...
          options.start = a;
          external = options.beautify[data.lexer[a]](options).replace(/\s+$/, "");
          build.push(external);
          if (levels[options.iterator] > -1 && externalIndex[a] > a) {
            build.push(nl(levels[options.iterator]));
          }
          a = options.iterator;
        }
      }
