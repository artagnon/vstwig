export function markupLexer(lexData: LexerData): data {
  ...
  const parse: parse = lexData.parse,
    ...
    asl: number = attributeSortList.length;
  //pads certain template tag delimiters with a space
  function bracketSpace(input: string): string {
  ...
  function recordPush(target: data, record: record, structure: string): void {
    if (target === data) {
  ...
  function tagName(el: string): string {
    let space: number = 0,
      name: string = "";
    const reg: RegExp = /^((\{|<)((%-?)|\{-?)=?\s*)/;
    if (typeof el !== "string") {
  ...
  function fixHtmlEnd(element: string, end: boolean): void {
    ...
    while (a < c) {
      ...
      if (quote === "") {
        if (lex[0] + lex[1] === "<!" && ltype !== "cdata") {
          ...
          if (b[a] !== ">" && b[a + 1] === "<") {
            lexData.parseerror = `SGML tag ${lex.join("")} is missing termination with '[' or '>'.`;
            break;
          }
        }
